// components/Home.js
import React, {Component } from 'react';
import Carouselpic from './Carousel';
import * as ReactBootstrap from "react-bootstrap";
import Row from 'react-bootstrap/Row';
import Container from 'react-bootstrap/Container';
import Col from 'react-bootstrap/Col';

import Simplemap from './Googlemap.js';


class Home extends Component {
  	render() {
  		return (

  		<div className="Home">
  			<h1 style={{'fontSize': "90px"}}>Room(me)</h1>
  			<h4>A modern, safe and convenient tool for ideal living situations</h4>
  			
    		

			<div className="Test">
				<Carouselpic />
			</div>

		<Container>
			<Row>
			<Col>  
				<h1 style={{'padding': '10px 0px 0px 0px', 'fontSize': '30px'}}>Currently available in the Bay Area</h1>
				<Simplemap />
			
			</Col>
			

			<Col>
				<ReactBootstrap.CardGroup>
 			 	<ReactBootstrap.Card>
   			 	<ReactBootstrap.Card.Img variant="top" src="data:image/png;base64," />
    			<ReactBootstrap.Card.Body>
     		 	<ReactBootstrap.Card.Title> Michael -SF </ReactBootstrap.Card.Title>
      			<ReactBootstrap.Card.Text style={{'fontSize': "15px"}}>
        		"Coming from a different state Room(me) made it easy to find a comfortable living situation risk free."
      			</ReactBootstrap.Card.Text>
    			</ReactBootstrap.Card.Body>
    			<ReactBootstrap.Card.Footer>
      			<small style={{'fontSize': "10px"}} className="text-muted">Last updated 3 mins ago</small>
    			</ReactBootstrap.Card.Footer>
  				</ReactBootstrap.Card>
  				<ReactBootstrap.Card>
    			<ReactBootstrap.Card.Img variant="top" src="https://inspireducation.org.pk/wp-content/uploads/2019/06/person-girl-flat.png" />
    			<ReactBootstrap.Card.Body>
      			<ReactBootstrap.Card.Title>Vanessa -SJ</ReactBootstrap.Card.Title>
      			<ReactBootstrap.Card.Text style={{'fontSize': "15px"}}>
      			"Through Room(me) I gained not only a roommate, but a life long friend"
      			</ReactBootstrap.Card.Text>
    			</ReactBootstrap.Card.Body>
    			<ReactBootstrap.Card.Footer>
      			<small style={{'fontSize': "10px"}} className="text-muted">Last updated 11 mins ago</small>
    			</ReactBootstrap.Card.Footer>
  				</ReactBootstrap.Card>
  				<ReactBootstrap.Card>
    			<ReactBootstrap.Card.Img  variant="top" src="https://cdn2.iconfinder.com/data/icons/circle-avatars-1/128/050_girl_avatar_profile_woman_suit_student_officer-512.png" />
    			<ReactBootstrap.Card.Body>
      			<ReactBootstrap.Card.Title>Sam -SF</ReactBootstrap.Card.Title>
      			<ReactBootstrap.Card.Text style={{'fontSize': "15px"}}>
        		"No more craigslist Nightmares! Will recommend to friends!"
      			</ReactBootstrap.Card.Text>
    			</ReactBootstrap.Card.Body>
    			<ReactBootstrap.Card.Footer>
      			<small style={{'fontSize': "10px"}} className="text-muted">Last updated 20 mins ago</small>
    			</ReactBootstrap.Card.Footer>
  				</ReactBootstrap.Card>
				</ReactBootstrap.CardGroup>
			</Col>
			</Row>
		</Container>

		</div>

		


		
		);
  	}
};

export default Home;